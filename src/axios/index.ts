...
const transform: AxiosTransform = {
  ...
  beforeRequestHook: (config, options) => {
    const { apiUrl, joinPrefix, joinParamsToUrl, formatDate, joinTime = true, ingoreApiUrl = false } = options;
    if (joinPrefix) {
      config.url = `${config.url}`;
    }
    if (!ingoreApiUrl && apiUrl && isString(apiUrl)) {
    ...
    deepMerge(
      {
        ...
        requestOptions: {
          ...
          // 接口地址
          apiUrl: (import.meta as any).env.VITE_APP_HOST,
          // apiUrl: globSetting.apiUrl,
          // 接口拼接地址
          urlPrefix: (import.meta as any).env.VITE_API_URL_PREFIX,
          //  是否加入时间戳
          joinTime: true,
          // 忽略重复请求
